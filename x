...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        local predictionMultiplier = getgenv().TriggerBot.Settings.Preds.GroundPredictionMultiplier  -- Default for ground

        -- Check if the player is in the air
        if character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            predictionMultiplier = getgenv().TriggerBot.Settings.Preds.AirPredictionMultiplier  -- Air prediction multiplier
        end
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            -- Adjust the prediction based on ping if AutoPred is enabled
            ...
            if getgenv().CamLock.Normal.AutoPred then
                -- Check the ping range and set the prediction factor accordingly
                if ping >= getgenv().PlayerPredictions.LowPing.Range[1] and ping < getgenv().PlayerPredictions.LowPing.Range[2] then
                    getgenv().CamLock.Normal.Prediction = getgenv().PlayerPredictions.LowPing.PredictionFactor
                elseif ping >= getgenv().PlayerPredictions.ModeratePing.Range[1] and ping < getgenv().PlayerPredictions.ModeratePing.Range[2] then
                    getgenv().CamLock.Normal.Prediction = getgenv().PlayerPredictions.ModeratePing.PredictionFactor
                elseif ping >= getgenv().PlayerPredictions.AveragePing.Range[1] and ping < getgenv().PlayerPredictions.AveragePing.Range[2] then
                    getgenv().CamLock.Normal.Prediction = getgenv().PlayerPredictions.AveragePing.PredictionFactor
                elseif ping >= getgenv().PlayerPredictions.HigherPing.Range[1] and ping < getgenv().PlayerPredictions.HigherPing.Range[2] then
                    getgenv().CamLock.Normal.Prediction = getgenv().PlayerPredictions.HigherPing.PredictionFactor
                elseif ping >= getgenv().PlayerPredictions.HighPing.Range[1] and ping < getgenv().PlayerPredictions.HighPing.Range[2] then
                    getgenv().CamLock.Normal.Prediction = getgenv().PlayerPredictions.HighPing.PredictionFactor
                elseif ping >= getgenv().PlayerPredictions.ExtremePing.Range[1] and ping < getgenv().PlayerPredictions.ExtremePing.Range[2] then
                    getgenv().CamLock.Normal.Prediction = getgenv().PlayerPredictions.ExtremePing.PredictionFactor
                else
                    -- Fallback if the ping doesn't match any of the defined ranges
                    getgenv().CamLock.Normal.Prediction = getgenv().CamLock.Normal.Prediction
                end
            end
...
-- // Get Velocity Function with Auto Prediction // --
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        -- Check if auto prediction is enabled and adjust the prediction value based on the ping
        if getgenv().Silent.Startup.AutoPrediction then
            if ping >= getgenv().PlayerPredictions.LowPing.Range[1] and ping < getgenv().PlayerPredictions.LowPing.Range[2] then
                getgenv().Silent.Startup.Prediction = getgenv().PlayerPredictions.LowPing.PredictionFactor
            elseif ping >= getgenv().PlayerPredictions.ModeratePing.Range[1] and ping < getgenv().PlayerPredictions.ModeratePing.Range[2] then
                getgenv().Silent.Startup.Prediction = getgenv().PlayerPredictions.ModeratePing.PredictionFactor
            elseif ping >= getgenv().PlayerPredictions.AveragePing.Range[1] and ping < getgenv().PlayerPredictions.AveragePing.Range[2] then
                getgenv().Silent.Startup.Prediction = getgenv().PlayerPredictions.AveragePing.PredictionFactor
            elseif ping >= getgenv().PlayerPredictions.HigherPing.Range[1] and ping < getgenv().PlayerPredictions.HigherPing.Range[2] then
                getgenv().Silent.Startup.Prediction = getgenv().PlayerPredictions.HigherPing.PredictionFactor
            elseif ping >= getgenv().PlayerPredictions.HighPing.Range[1] and ping < getgenv().PlayerPredictions.HighPing.Range[2] then
                getgenv().Silent.Startup.Prediction = getgenv().PlayerPredictions.HighPing.PredictionFactor
            elseif ping >= getgenv().PlayerPredictions.ExtremePing.Range[1] and ping < getgenv().PlayerPredictions.ExtremePing.Range[2] then
                getgenv().Silent.Startup.Prediction = getgenv().PlayerPredictions.ExtremePing.PredictionFactor
            else
                getgenv().Silent.Startup.Prediction = OldPredictionY  -- Fallback to default if ping is too high
            end
        end
        
        -- If the resolver is enabled, adjust the prediction based on velocity or movement direction
